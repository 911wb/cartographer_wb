...
// 真正开始添加一条新轨迹，入参:所有传感器topic的集合、lua文件部分参数
// 为什么不直接给map_builder添加轨迹，而构建一个bridge，因为在bridge中添加了一些逻辑判断
int MapBuilderBridge::AddTrajectory(
    const std::set<cartographer::mapping::TrajectoryBuilderInterface::SensorId>& expected_sensor_ids,
    const TrajectoryOptions& trajectory_options) 
{
  // step1 真正开始给map_builder_添加一条新的轨迹, 返回新轨迹的id,需要传入一个函数
const int trajectory_id = map_builder_->AddTrajectoryBuilder(
    expected_sensor_ids,
    trajectory_options.trajectory_builder_options,// lua文件参数
    // [this]表示map_builder_bridge类中的所有成员都可以访问，匿名函数参数：(轨迹构建器传入的参数)
    [this](const int trajectory_id, const ::cartographer::common::Time time,
            const Rigid3d local_pose,::cartographer::sensor::RangeData range_data_in_local,// 点云数据
            const std::unique_ptr<const ::cartographer::mapping::TrajectoryBuilderInterface::InsertionResult>)
    {
      // 轨迹构建器的参数又给到下面的函数，保存local slam 的结果数据 5个参数实际只用了4个
  ...
  // step2 为这个新轨迹添加一个数据类型转换器，将ros格式转换为carto格式,
  // 左值类型std::unique_ptr<SensorBridge>，调用SensorBridge构造函数初始化
  sensor_bridges_[trajectory_id] = absl::make_unique<SensorBridge>(
      trajectory_options.num_subdivisions_per_laser_scan,
      trajectory_options.tracking_frame,
      node_options_.lookup_transform_timeout_sec, 
      tf_buffer_,
      // 获取轨迹构建器的地址，将转换后的数据送给轨迹构建器
    ...
    cartographer_ros_msgs::SubmapQuery::Response& response) 
{
  ...
  // 遍历response_proto，将response_proto中的地图栅格值存入到response中
