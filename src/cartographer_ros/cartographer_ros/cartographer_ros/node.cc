 ...
 * @param[in] handler 函数指针(返回普通类型), 接收传入的函数的地址(函数名字或&函数名字)
 ...
 */
// c++11:成员函数指针的初始化和声明：int (Myclass::* pFun)(int, int) = &Myclass::Fun;
 ...
 * @param[in] tf_buffer 缓存区接收到的数据
 * @param[in] collect_metrics 是否启用metrics,默认不启用
 */
// 初始化参数列表
Node::Node(
    const NodeOptions& node_options,
    std::unique_ptr<cartographer::mapping::MapBuilderInterface> map_builder,// 左值
    tf2_ros::Buffer* const tf_buffer, const bool collect_metrics)
    : node_options_(node_options), 
    // MapBuilderBridge的对象初始化：实参map_builder(如果是MapBuilder类的智能指针)与形参(map_builder指向的类型MapBuilderInterface)不匹配，
    // notice: 因为实参是形参的子类(继承多态,形参是父类指针可以指向子类对象)，class MapBuilder : public MapBuilderInterface
    // 构造函数初始化，std::move(map_builder)右值
    map_builder_bridge_(node_options_, std::move(map_builder), tf_buffer) 
{
  ...
  // step1 创建发布对象，声明需要发布的topic
  // advertise<发布的消息类型>(话题，队列中最大保存的消息数，超出此阀值时，先进的先销毁)

  // 发布"SubmapList" ： header ,submap_trajetory_id、submap_index、pose
  submap_list_publisher_ =
      node_handle_.advertise<::cartographer_ros_msgs::SubmapList>(
          kSubmapListTopic, kLatestOnlyPublisherQueueSize);

  // 发布轨迹"trajectory_node_list" 可视化
  trajectory_node_list_publisher_ =
      node_handle_.advertise<::visualization_msgs::MarkerArray>(
          kTrajectoryNodeListTopic, kLatestOnlyPublisherQueueSize);
  // 发布"landmark_poses_list" 可视化
  landmark_poses_list_publisher_ =
      node_handle_.advertise<::visualization_msgs::MarkerArray>(
          kLandmarkPosesListTopic, kLatestOnlyPublisherQueueSize);
  // 发布"constraint_list" 可视化
  ...
  // 服务端(客户端会过来调用)，node_handle_.advertiseService(参数1为服务名称，参数2为回调函数:处理客户端请求的数据，并返回响应数据给客户端，参数3：this)
  // step2 声明发布对应名字的ROS服务, 并将服务的发布器放入到vector容器中

  // 客户端访问服务端(client.call)，提交数据给服务端处理，服务端经过回调函数处理好后返回给客户端
  // 参数服务器，一般适用于存在数据共享的一些应用场景： Talker 设置参数，Listener 获取参数

  //   this:入参是对象的函数，就需要把对象也作为入参，如果是普通函数，就不用传对象
  
  // 服务话题名称："submap_query"，client->call(srv)客户端发送srv请求，服务端处理请求，并更新响应结果
  service_servers_.push_back(node_handle_.advertiseService(
      kSubmapQueryServiceName, &Node::HandleSubmapQuery, this));// 服务名称、回调函数

  // 处理轨迹信息"trajectory_query"，并未调用该回调函数
  service_servers_.push_back(node_handle_.advertiseService(
      kTrajectoryQueryServiceName, &Node::HandleTrajectoryQuery, this));

  // 服务名称"start_trajectory"
  // 用户在终端中通过命令来调用回调函数，只有一个机器人时，并没有客户端调用该回调函数
  service_servers_.push_back(node_handle_.advertiseService(
      kStartTrajectoryServiceName, &Node::HandleStartTrajectory, this));

  // 处理结束轨迹信息"finish_trajectory"，并未调用该回调函数
  service_servers_.push_back(node_handle_.advertiseService(
      kFinishTrajectoryServiceName, &Node::HandleFinishTrajectory, this));

  // 保存轨迹 "write_state"，并未调用该回调函数
  service_servers_.push_back(node_handle_.advertiseService(
      kWriteStateServiceName, &Node::HandleWriteState, this));

  // 获取轨迹状态"get_trajectory_states"，并未调用该回调函数
  service_servers_.push_back(node_handle_.advertiseService(
      kGetTrajectoryStatesServiceName, &Node::HandleGetTrajectoryStates, this));
  // 度量 "read_metrics"，，并未调用该回调函数
  service_servers_.push_back(node_handle_.advertiseService(
      kReadMetricsServiceName, &Node::HandleReadMetrics, this));


  // step3 创建点云的发布对象,发布帧间匹配后的点云，topic:"scan_matched_points2"
  scan_matched_point_cloud_publisher_ =
      node_handle_.advertise<sensor_msgs::PointCloud2>(
          kScanMatchedPointCloudTopic, kLatestOnlyPublisherQueueSize);

  // step4 进行定时器与函数的绑定, 定时发布数据(定时执行回调函数PublishSubmapList)
  // (定时器回调函数之间的时间间隔, 定时器回调函数、类方法或者函数子对象)
...
/**
 * @brief 新增一个位姿估计器(IMU和里程计的融合)，只做的先验
 * 
 * @param[in] trajectory_id 轨迹id
                           ...
                           const TrajectoryOptions& options) 
{
  ...
  */
  ...
  // std::map<int, ::cartographer::mapping::PoseExtrapolator> extrapolators_需要三个参数才能初始化

  extrapolators_.emplace(
      std::piecewise_construct, // 分次生成tuple的标志常量
      std::forward_as_tuple(trajectory_id),// 本来返回的是tuple类型，现在是int
      std::forward_as_tuple(
          ::cartographer::common::FromSeconds(kExtrapolationEstimationTimeSec),
          gravity_time_constant));// 本来返回的是tuple类型，现在是PoseExtrapolator
}
                             ...
                             const TrajectoryOptions& options) 
{
  ...
  sensor_samplers_.emplace(
      ...
      std::forward_as_tuple(
          // 五种采样器的频率作为TrajectorySensorSamplers构造函数的参数来初始化
...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent& timer_event) 
{
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) 
  {
    ...
    stamped_transform.header.stamp =
        node_options_.use_pose_extrapolator// 默认不使用 x0？x1：x2；
            ? ToRos(now)// x0为真
            : ToRos(trajectory_data.local_slam_data->time);// xo为假：前端时间戳
    ...
    const Rigid3d tracking_to_local_3d =
        node_options_.use_pose_extrapolator
            ...
            : trajectory_data.local_slam_data->local_pose;// 前端位姿发布tf
    
    // 获取当前位姿在local坐标系下的坐标 c++11: lambda
    ...
    // 求得当前位姿在map下的坐标，local坐标系下的位姿转换到map坐标系
...
Node::ComputeExpectedSensorIds(const TrajectoryOptions& options) const 
{
  ...
  // 调用set构造函数，创建一个集合expected_topics
  std::set<SensorId> expected_topics;
  // Subscribe to all laser scan, multi echo laser scan, and point cloud topics.

  // 如果只有一单线雷达， 那订阅的topic就是topic
  // 如果是多个单线雷达, 那订阅的topic就是topic_1,topic_2, 依次类推
  for (const std::string& topic :
      // 返回topics容器
       ComputeRepeatedTopicNames(kLaserScanTopic, options.num_laser_scans)) // 入参:("scan",0)，返回话题名称
  {
    // 结构体初始化，并插入
  ...
  for (const std::string& topic : ComputeRepeatedTopicNames(
           kMultiEchoLaserScanTopic, options.num_multi_echo_laser_scans)) // 入参：("echoes",0)
  {
    expected_topics.insert(SensorId{SensorType::RANGE, topic});
  }
  ...
  // 多线雷达
  for (const std::string& topic :
       ComputeRepeatedTopicNames(kPointCloud2Topic, options.num_point_clouds)) // 入参:("points2",1)
  {
    expected_topics.insert(SensorId{SensorType::RANGE, topic});
  }
...
{
  LOG(INFO) << "-----------------------start a new trajectory----------------------- ";

  // 返回所有传感器类型和对应的topic名字
  const std::set<cartographer::mapping::TrajectoryBuilderInterface::SensorId>
      expected_sensor_ids = ComputeExpectedSensorIds(options);

  // 调用map_builder_bridge的AddTrajectory, 添加一个轨迹，返回新生成的轨迹id
  // 返回轨迹id，入参：所有传感器topic的集合、lua文件部分参数
  const int trajectory_id =
      map_builder_bridge_.AddTrajectory(expected_sensor_ids, options);

  // 新增一个位姿估计器，作用是：恒速模型？递推位姿
  AddExtrapolator(trajectory_id, options);

  // 新生成一个传感器数据采样器，作用是：数据频率高需要降采样
  AddSensorSamplers(trajectory_id, options);

  // 订阅话题与注册回调函数，开始订阅，随时准备接收客户端数据，ros的底层客户端会调用carto的服务端，将传感器数据作为请求发送给服务端 ??
  LaunchSubscribers(options, trajectory_id);

  // 创建了一个3s执行一次的定时器,由于oneshot=true, 所以只执行一次MaybeWarnAboutTopicMismatch
...
void Node::LaunchSubscribers(const TrajectoryOptions& options,
                             const int trajectory_id) 
{
  // 单线雷达数据laser_scan 的订阅与注册回调函数, 多个laser_scan的topic(订阅的topic就是scan_1,scan_2), 共用同一个回调函数
  for (const std::string& topic :
       ComputeRepeatedTopicNames(kLaserScanTopic, options.num_laser_scans)) 
  {
    // 把返回值和topic放在一起，共同添加到subscribers_的值，键：trajectory_id
    subscribers_[trajectory_id].push_back(
        // 模板函数，入参：函数地址、轨迹id、话题、节点、node_this，返回：订阅对象<订阅消息类型>(topic、队列长度、回调函数)
        {SubscribeWithHandler<sensor_msgs::LaserScan>(
             &Node::HandleLaserScanMessage, trajectory_id, topic, &node_handle_,this),
         topic});
  }

  // 多回声波雷达数据multi_echo_laser_scans的订阅与注册回调函数
  for (const std::string& topic : ComputeRepeatedTopicNames(
  ...
  // required.
  // imu数据 的订阅与注册回调函数,只有一个imu的topic
  if (node_options_.map_builder_options.use_trajectory_builder_3d() ||
      (node_options_.map_builder_options.use_trajectory_builder_2d() &&
       options.trajectory_builder_options.trajectory_builder_2d_options()
           .use_imu_data())) 
  ...
  // odometry数据 的订阅与注册回调函数,只有一个odometry的topic
  if (options.use_odometry) 
  {
    subscribers_[trajectory_id].push_back(
  ...
  // landmarks数据 的订阅与注册回调函数,只有一个landmarks的topic
  if (options.use_landmarks) 
  {
    subscribers_[trajectory_id].push_back(
...
// 检查TrajectoryOptions是否存在2d或者3d轨迹的配置信息，入参：配置文件部分参数
...
{
  cartographer_ros_msgs::StatusResponse status_response;
  // step1 检查 trajectory_id 是否在 正在结束的轨迹集合中
  ...
  // step2 检查这个轨迹是否存在, 如果存在则检查这个轨迹是否是ACTIVE状态
  ...
  // step3 如果这个轨迹存在subscribers, 则先关闭subscriber
  ...
  // step4 调用cartographer中的map_builder的FinishTrajectory()进行轨迹的结束
...
// 使用默认topic名字开始一条轨迹,也就是开始slam , 入参:部分lua文件参数
                                 ...
                                 const nav_msgs::Odometry::ConstPtr& msg) 
{
  ...
  // sensor_bridge() 返回指向SensorBrid的指针
  auto sensor_bridge_ptr = map_builder_bridge_.sensor_bridge(trajectory_id);
  // 数据格式转换
  auto odometry_data_ptr = sensor_bridge_ptr->ToOdometryData(msg);
  // extrapolators_使用里程计数据进行位姿预测，融合IMU和里程计，输出推测位姿
  if (odometry_data_ptr != nullptr) 
  {
    // 转换后的里程计数据，添加给位姿推测器
    // use_pose_extrapolators = false ，如果是true则使用extrapolators_推测出的位姿
...
void Node::HandleImuMessage(const int trajectory_id,
                            const std::string& sensor_id,
                            const sensor_msgs::Imu::ConstPtr& msg)// 函数重载
{
  absl::MutexLock lock(&mutex_);
  ...
  // extrapolators_使用IMU数据进行位姿预测，融合IMU和里程计，输出推测位姿
  if (imu_data_ptr != nullptr)
  {
    // 发布tf的时候可以选择，位姿推测器计算出的位姿或者前端计算出的位姿
    // use_pose_extrapolators = false ，如果是true则使用extrapolators_推测出的位姿
    // 作为最优位姿，但没有经过雷达校准，只是先验位姿，所以始终是false
    // 转换后的IMU数据，添加给位姿推测器
    extrapolators_.at(trajectory_id).AddImuData(*imu_data_ptr);
  }
  sensor_bridge_ptr->HandleImuMessage(sensor_id, msg);// 函数重载
}

//回调函数domsg
void Node::HandleLaserScanMessage(const int trajectory_id,
                                  const std::string& sensor_id,// 话题
                                  const sensor_msgs::LaserScan::ConstPtr& msg)
{
  // 每一种传感器的都有自己的回调函数，防止回调函数之间互相访问数据
  absl::MutexLock lock(&mutex_);
  // 根据配置,是否将传感器数据跳过
  ...
  // 调用SensorBridge的传感器处理函数进行数据处理
  // 数据转换器：sensor_bridge，返回对应轨迹id的指向SensorBridge的指针
  map_builder_bridge_.sensor_bridge(trajectory_id)
    ...
    const ::ros::WallTimerEvent& unused_timer_event) 
{
  ...
  // notice: 使用ros的master的api进行topic名字的获取
  ...
  for (const auto& entry : subscribers_) 
  {
    ...
    for (const auto& subscriber : entry.second) 
    {
      ...
      // 在发布的topic中查找订阅topic的个数，没有找到就为0
