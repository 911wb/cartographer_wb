...
/**
 * @brief 在构造函数中调用AddLogSink(), 将ScopedRosLogSink类注册到glog中，使用send()生成消息
 */
// 初始化参数列表 ?? this，this指针指向的是调用该构造函数的对象 主要的功能是添加或移除一个LogSink作为日志数据的使用者
...
/**
 * @brief 重载了send()方法, 使用ROS_INFO进行glog消息的输出，自定义消息格式
 * 
