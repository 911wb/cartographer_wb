...
Node::Node(const double resolution, const double publish_period_sec)
    : resolution_(resolution),
      // 初始化SubmapQuery服务的客户端，服务名称kSubmapQueryServiceName
      client_(node_handle_.serviceClient<::cartographer_ros_msgs::SubmapQuery>(
          kSubmapQueryServiceName)),
      // 初始化订阅方：订阅 submap_list topic, 的子图数据
      submap_list_subscriber_(node_handle_.subscribe(
          ...
          boost::function<void(
              ...
              {
                // 处理SubmapList话题消息的回调函数
                HandleSubmapList(msg);
              }))),

      // 初始化发布端：声明 map 发布器，发布的消息类型：占用的栅格地图，接收对象：Rviz、navigation
      // 如果是true，会保存发布方的最后一条消息，并且新的订阅对象连接到发布方时，发布方会将这条消息发送给订阅者
    ...
    const cartographer_ros_msgs::SubmapList::ConstPtr& msg) 
{
  ...
  for (const auto& submap_msg : msg->submap) 
  {
    ...
    // step1 获取格式为io::SubmapTextures的地图栅格数据
    // 客户端调用服务端，服务端响应，返回的响应是纹理图(栅格数据)
    ...
    // step2 生成surface(submap_slice.cairo_data), 返回指向新创建图像的指针
    submap_slice.surface = ::cartographer::io::DrawTexture(
        fetched_texture->pixels.intensity, // 地图栅格值
        fetched_texture->pixels.alpha,/// 地图栅格透明度
        fetched_texture->width, fetched_texture->height,
        &submap_slice.cairo_data);// 赋值cairo_data，返回指向它的智能指针
  } // end for
...
void Node::DrawAndPublish(const ::ros::WallTimerEvent& unused_timer_event) 
{
  ...
  //  step3 生成PaintSubmapSlicesResult, 绘制栅格地图的cairo图像，
  auto painted_slices = PaintSubmapSlices(submap_slices_, resolution_);
  
  // step4  notice: 根据painted_slices图形来生成ros格式栅格地图，适配amcl和costmap
  std::unique_ptr<nav_msgs::OccupancyGrid> msg_ptr = CreateOccupancyGridMsg(
      painted_slices, resolution_, last_frame_id_, last_timestamp_);
  
  //  step5 发布ros格式栅格地图数据，其话题为map，notice: 1.发给RVIZ可视化， 2.发给ros的导航模块做路径规划
...
// notice: 启动一个新的节点(cartographer_occupancy_grid_node)生成ros格式的二维栅格地图地图
int main(int argc, char** argv) 
{
  ...
  // 这个Node类指的是上边的类,与node.cc无关，入参：地图分辨率，地图发布的时间间隔
