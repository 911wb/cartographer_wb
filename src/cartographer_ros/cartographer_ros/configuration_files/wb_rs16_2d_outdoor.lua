...
options = {
  ...
  tracking_frame = "imu_link",              -- 将所有传感器数据通过坐标变换转换到这个坐标系下
  ...
  use_odometry = true,                     -- 是否订阅里程计传感器,如果使用要求一定要有odom的tftree

  use_nav_sat = true,                      -- 是否订阅gps
  use_landmarks = false,                    -- 是否订阅landmark
  num_laser_scans = 0,                      -- 是否订阅单线激光数据
  num_multi_echo_laser_scans = 0,           -- 是否订阅多回声波雷达数据
  num_subdivisions_per_laser_scan = 1,      -- 1帧数据被分成几次处理,一般为1
  -- 可以既订阅单线，又可以订阅多线，但是min_z会矛盾，可以设置为-0.1
  num_point_clouds = 1,                     -- 是否订阅点云数据，是否使用多线激光雷达
  ...
  fixed_frame_pose_sampling_ratio = 1.,  -- 如果某个传感器的不准，可以将采样频率降低
...
-- 建图效果调整
-- 点云和地图匹配的程度，位姿残差×权重，权重越大，越相信匹配结果，一般是10
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.occupied_space_weight = 1.
-- 帧间匹配位姿与先验位姿的残差×权重，权重越大，越相信先验位姿的结果
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 1.
-- imu不好时，先验位姿不好，可以调小该权重
...
-- 一个子图插入160个节点，这个数越小，生成的子图越多(一堆小子图)，构造多分辨率地图时，总的子图变得更多，占用内存大
