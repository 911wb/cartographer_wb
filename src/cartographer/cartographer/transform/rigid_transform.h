...
class Rigid3 
{
 public:
  ...
  /*
   T = [R t] T¯¹ = [R^-1  -R^-1 * t]
       [0 1]        [0         1    ] 
       Pw = Ra*Pa + ta --> Pa = Ra¯¹(Pw - ta) = Ra¯¹*Pw + Ra¯¹*ta
       Pw = T * Pa --> T¯¹
   R是旋转矩阵, 特殊正交群, 所以R^-1 = R^T
  */
...
/*
 lhs是全局坐标系下的位姿, rhs是全局坐标系下的坐姿变动量
 lhs.rotation() * rhs.translation() + lhs.translation() 的意思是
 将 rhs 转换成 lhs自身坐标系下的位姿变动量 再与lhs的坐标相加 得到 lhs 在全局坐标系下的新的位姿
 |y         --> y\  /x  --> y\  /x
 |_____x          \/          \/
 Pw              Pb           Pa
        world-->B:(tB, RB)   world-->A:(tA,RA)
  t' = RA*tB + tA
  R' = RA*RB

  A-->B:
  Pa是A的原点，在世界坐标系下的坐标为Paw、Pa是A的原点，在世界坐标系下的坐标为Pbw：
  
  Pw = Rwa*Pa + ta  --> Pa = Rwa¯¹(Pw - ta) --> Pa = Twa¯¹ Pw
  Pw = Rwb*Pb + tb  --> Pb = Rwb¯¹(Pw - tb) --> Pb = Twb¯¹ Pw
  Pb = Rwb¯¹(Pw - tb) = Rwb¯¹(Rwa*Pa + ta - tb)
  = Rwb¯¹Rwa*Pa + Rwb¯¹(ta - tb) = Rba*Pa + t
  Rba = Rwb¯¹Rwa = ξa - ξb,  t = Rwb¯¹(ta - tb)
  Pb = Rba*Pa + t
  
*/

template <typename FloatType>
Rigid3<FloatType> operator*(const Rigid3<FloatType>& lhs,// 左
                            const Rigid3<FloatType>& rhs) 
{
  return Rigid3<FloatType>(
      lhs.rotation() * rhs.translation() + lhs.translation(),//t' = Rl*tr + tl
      (lhs.rotation() * rhs.rotation()).normalized());// R' = Rl*Rr
}
