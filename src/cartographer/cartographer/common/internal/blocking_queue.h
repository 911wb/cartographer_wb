...
为什么要使用生产者消费者模式, 顺序执行不就可以了吗？生产者消费者到底有什么意义？
  ...
  2.并发 （异步）
    生产者直接调用消费者, 两者是同步（阻塞）的, 如果消费者吞吐数据很慢, 这时候生产者白白浪费大好时光和数据。
    而使用这种模式之后, 生产者将数据丢到缓冲区, 继续生产, 完全不依赖消费者, 程序执行效率会大大提高。

  3.复用
    通过将生产者类和消费者类独立开来, 可以对生产者类和消费者类进行独立的复用与扩展
  生产者可以不同，消费者可以相同
 */
...
class BlockingQueue
{
 public:
  ...
  // 将值压入队列. 如果队列已满, 则阻塞达到缓冲的效果
  void Push(T t) 
  {
    // 首先定义判断函数
    const auto predicate = [this]() EXCLUSIVE_LOCKS_REQUIRED(mutex_) 
    {
      return QueueNotFullCondition();// 如果队列未满, 则返回true
    };

    // absl::Mutex的更多信息可看: https://www.jianshu.com/p/d2834abd6796
    // absl官网: https://abseil.io/about/

    // 如果数据满了, 生产者就进行等待，让消费者先去消费队列中的数据，达到缓冲的效果
    absl::MutexLock lock(&mutex_);
    // Condition返回结果是false则等待
  ...
  T Pop() 
  {
    const auto predicate = [this]() EXCLUSIVE_LOCKS_REQUIRED(mutex_) 
    {
      return !QueueEmptyCondition();// 如果队列为空, 则返回!true
    };

    // 等待直到数据队列中至少有一个数据
    absl::MutexLock lock(&mutex_);
    // 如果消费者使用太快导致队列中没有数据了，要等待生产者生产数据，队列不为空，才能删除数据
    mutex_.Await(absl::Condition(&predicate));
    // 取出第一个值复制给t
