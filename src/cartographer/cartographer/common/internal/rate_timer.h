...
class RateTimer {
 public:
  ...
  void Pulse(common::Time time) 
  {
    ...
    // 结构体初始化并放入队列中，参数：传感器时间，c++当前时间
    events_.push_back(Event{time, ClockType::now()});
    // 删除队列头部数据,直到队列中最后与最前间的时间间隔小于window_duration_=15s
    while (events_.size() > 2 &&
           (events_.back().wall_time - events_.front().wall_time) > window_duration_) 
    {
      // 删除队列头部
      events_.pop_front();
    }
