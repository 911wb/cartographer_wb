...
ThreadPool::ThreadPool(int num_threads)// 4
{
  ...
  {
    // pool_:线程池 == 多个子线程组成的容器 ，构造线程放到容器中 std::vector<std::thread> pool_
    // 新建四个线程，每个线程都执行DoWork()函数
...
void ThreadPool::NotifyDependenciesCompleted(Task* task)
{
  ...
  // 当前任务的共享指针加入到任务队列中，在DoWork()中等待执行
...
void ThreadPool::DoWork() 
{
  ...
  for (;;) 
  {
...

    {
      ...
      else if (!running_)// 如果任务为空并且running为false
      {
        return;// 循环结束 
      }
    }
    // 如果task为空，代表没有任务，不会往下执行
    CHECK(task);
