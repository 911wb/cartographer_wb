...
// 设置本Task需要执行的任务 （函数） 
// 状态: NEW
// 设置本Task需要执行的任务函数，lambda表达式的名字为work_item
...
// 本任务依赖的任务完成了, 可以将本任务加入到线程池的待处理列表(task_queue_)中了
...
void Task::Execute() 
{
  ...
  // 遍历本任务的依赖任务，通知依赖本任务的其他任务：本任务执行完了
  for (Task* dependent_task : dependent_tasks_) 
  {
    // dependent_task所依赖的任务(执行完了)-1，执行dependent_task
    dependent_task->OnDependenyCompleted();
  }
}
