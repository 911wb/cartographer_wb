 ...
 * @param[in] callback 2个参数的回调函数, 实际是HandleCollatedSensorData()函数
...
{ 
  ...
  {
    // QueueKey重载<，来排序，每一个话题产生一个队列key
    const auto queue_key = QueueKey{trajectory_id, sensor_id};
    // 遍历传感器id，每一个传感器都有各自的队列取存储消息 
    // 通过callback函数(lambda)把数据送出给轨迹构建器
    queue_.AddQueue(queue_key,// 队列名字如：imu、里程计、雷达
                    // callback2个参数的回调函数, 实际是HandleCollatedSensorData()函数
