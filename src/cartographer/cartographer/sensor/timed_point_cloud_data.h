...
{
  common::Time time;        // 点云最后一个点的绝对时间
  // 雷达坐标系在IMU坐标系下的位置坐标
  Eigen::Vector3f origin;   // 以tracking_frame_到雷达坐标系的transaltion为原点
  TimedPointCloud ranges;   // 数据点的集合, 每个数据点包含xyz与time, time是负的
  // 'intensities' has to be same size as 'ranges', or empty.
  std::vector<float> intensities; // 强度是空的
};
...
struct TimedPointCloudOriginData 
{
  struct RangeMeasurement 
  {
    ...
    float intensity;                    // 强度值为空
    // 多个激光数据合并在一起后，对应不同的发射原点，每一个点云对应的发射原点
    size_t origin_index;                // 属于第几个origins的点
  };
  common::Time time;                    // 点云的时间
  // 多个雷达坐标系在IMU坐标系下的位置坐标
