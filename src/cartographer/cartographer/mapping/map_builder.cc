...
// c++11: 匿名命名空间, 作用域被限制在本文件内，使得该命名空间的所有内容，仅在本文件可见，其他文件无法调用
namespace { 
 ...
 * 
 * @param[in] options proto::MapBuilderOptions格式的 map_builder参数，lua文件参数
 */
MapBuilder::MapBuilder(const proto::MapBuilderOptions& options)
    // 调用两个构造函数，初始化两个对象 ，初始化线程池，使其一直循环
    : options_(options), thread_pool_(options.num_background_threads())// 4 
{
  // param:num_background_threads = 4, -- 后端线程数
  // 不能同时true、同时false
  ...
  {
    // sensor_collator_构造函数初始化, 实际使用这个
 ...
 * @param[in] trajectory_options lua文件轨迹的参数配置
 * @param[in] local_slam_result_callback 需要传入的回调函数(AddTrajectoryBuilder的lambda表达式)
...
int MapBuilder::AddTrajectoryBuilder(
    ...
    const proto::TrajectoryBuilderOptions& trajectory_options,
    // 回调函数 == OnLocalSlamResult == local_slam_result_callback_ 
    LocalSlamResultCallback local_slam_result_callback)
{
  // id是从零开始的, 所以新trajectory_id就是trajectory_builders_容器的size()

  // carto支持多个轨迹同时运行，一条轨迹存放到一个容器中
  ...
  if (trajectory_options.has_pose_graph_odometry_motion_filter())// false
  {
  ...
  // 前端包含了 Pose Extrapolator, Scan Matching, 生成submap 等

  // 如果使用3d的轨迹
  if (options_.use_trajectory_builder_3d())
  {
    // local_trajectory_builder(前端)的初始化，默认构造函数创建智能指针
    ...
    */
    ...
    // trajectory_id = trajectory_builders_.size();0->1，trajectory_builders_.at(0)
    // 往存放智能指针(指向基类TrajectoryBuilderInterface)的容器中添加指针(指向子类)
    // 调用构造函数创建匿名对象指针并存入trajectory_builders_
    trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
        trajectory_options, sensor_collator_.get(), 
        trajectory_id, expected_sensor_ids,
        // 返回完整3Dslam的智能指针
        CreateGlobalTrajectoryBuilder3D(
            std::move(local_trajectory_builder),// 3D前端
            trajectory_id, static_cast<PoseGraph3D*>(pose_graph_.get()),// 3D后端
            local_slam_result_callback, // 回调函数：OnLocalSlamResult
  ...
  else 
  {
    ...
    trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
        ...
        // notice: 全局轨迹构建器(2d的完整的slam，建图核心)，返回完整2Dslam的智能指针
        CreateGlobalTrajectoryBuilder2D(
            std::move(local_trajectory_builder),// 前端
            trajectory_id,// 轨迹id
            static_cast<PoseGraph2D*>(pose_graph_.get()),// 后端
            // 全局轨迹构建器有了结果后，会反馈数据给AddTrajectoryBuilder的lambda表达式 ??
            local_slam_result_callback,// 回调函数：OnLocalSlamResult
            pose_graph_odometry_motion_filter)));
  }
...
// API ，智能指针：MapBuilderInterface类型的对象指针CreateMapBuilder
std::unique_ptr<MapBuilderInterface> CreateMapBuilder(
    const proto::MapBuilderOptions& options) 
{
  // 调用构造函数，返回指向MapBuilder的智能指针
