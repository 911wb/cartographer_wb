...
/**
 * note: 地图坐标系可视化展示
 * ros的地图坐标系    cartographer的地图物理坐标系     cartographer地图的像素坐标系 
 * 
 * ^ y                            ^ x              0------> x
 * |                              |                |
 * |                              |                |
...
class MapLimits 
{
 public:
   ...
   * @brief 构造函数
   * 
   * @param[in] resolution 地图分辨率，lua文件中为 0.05
   * @param[in] max carto地图坐标系，左上角的坐标为地图坐标的最大值: origin + (x+50个栅格对应的物理长度, y + 50个栅格对应的物理长度)
  ...
  // 输入点的物理坐标，计算物理坐标点的像素索引(栅格数)
  Eigen::Array2i GetCellIndex(const Eigen::Vector2f& point) const 
  {
    // 分别返回像素坐标系中的(x,y)
    return Eigen::Array2i(
        // 物理长度/分辨率 = 栅格数，分辨率 = 物理长度/像素个数
  ...
  bool Contains(const Eigen::Array2i& cell_index) const 
  {
    ...
    return (Eigen::Array2i(0, 0) <= cell_index).all() &&
           (cell_index <
            Eigen::Array2i(cell_limits_.num_x_cells, cell_limits_.num_y_cells)).all();
  }

private:
  double resolution_;
  // cartographer地图坐标系左上角为坐标系的坐标的最大值，物理坐标
  Eigen::Vector2d max_;
