...
void ProbabilityGrid::SetProbability(const Eigen::Array2i& cell_index,// 二维像素坐标
                                     const float probability) // 该像素对应的占据栅格概率
{
...
// 使用查找表对指定栅格进行栅格值的更新，用空间换时间
bool ProbabilityGrid::ApplyLookupTable(const Eigen::Array2i& cell_index,// 一个点云的像素坐标
                                       const std::vector<uint16>& table)// 查找表
{
  ...
  // 当前一帧点云，是不会击中同一个栅格两次(除非距离很近)，多帧点云才会多次击中同一个栅格，
  // 下一帧点云来的时候栅格值就不大于kUpdateMarker
  if (*cell >= kUpdateMarker)
  {
    return false;//  更新过了不再进行更新了
  }
  ...
  // 更新栅格值，输入旧栅格值返回新的栅格值如：0.5->0.55： 18432 = table[16384] ??
  // 之前已经提前把odds * odds_old的结果(所有可能的更新结果)存下来了，用的时候再取出来，不用再花时间计算
  *cell = table[*cell];
