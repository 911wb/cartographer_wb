...
// 栅格是未知状态与odds状态下, 将更新时的所有可能结果预先计算出来
std::vector<uint16> ComputeLookupTableToApplyCorrespondenceCostOdds(float odds)// 1.22、0.96
{
  std::vector<uint16> result;
  result.reserve(kValueCount); // 32768

  // 当前cell是unknown情况下直接把odds转成value存进来
  // odds --> p_occu --> p_free --> value_grid：[1~32767]
  result.push_back(CorrespondenceCostToValue(ProbabilityToCorrespondenceCost(
                       ProbabilityFromOdds(odds))) +
                   kUpdateMarker);
                   //notice: 加上kUpdateMarker作为一个标志, 代表这个栅格已经被更新了，之后同一帧的点云再次更新该栅格的时候，发现其更新过了，就不在更新
  
  // 计算更新时 从1到32768的所有可能的 更新后的结果
  for (int cell = 1; cell != kValueCount; ++cell)
  {
    result.push_back(
        // [1~32767] --> p_free --> odds_old * odds = odds_new --> p_occu --> p_free --> value_grid：[1~32767]
