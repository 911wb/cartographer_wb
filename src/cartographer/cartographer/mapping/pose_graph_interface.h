...
class PoseGraphInterface {
 public:
  ...
  struct Constraint 
  {
    ...
    // 一片子图内有多个节点位姿
    SubmapId submap_id;  // 'i' in the paper：ci.
    NodeId node_id;      // 'j' in the paper：cj.
    ...
    // into submap 'i').
    /*
    节点位姿：local坐标系下tracking_frame的位姿，子图原点的位姿(起始位姿)

    子图内约束：
      一帧点云扫描匹配一个后验位姿，需要180帧点云生成一个子图，所以一个子图中有180个节点位姿
      180帧点云和这些节点位姿也属于该子图，该子图中的节点位姿(后验位姿)与子图原点(起始位姿)间的约束叫做，子图内约束。
    子图间约束：
      两个不同的子图，但是可以匹配得上，这两个不同子图之间的约束叫做闭环约束。
