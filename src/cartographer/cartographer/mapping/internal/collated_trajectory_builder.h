...
// 处理传感器数据, 使其按照时间排列, 然后传入GlobalTrajectoryBuilder前端
class CollatedTrajectoryBuilder : public TrajectoryBuilderInterface 
{
 public:
  ...
  CollatedTrajectoryBuilder(
      ...
      std::unique_ptr<TrajectoryBuilderInterface> wrapped_trajectory_builder);// 完整slam
  ...
  CollatedTrajectoryBuilder& operator=(const CollatedTrajectoryBuilder&) =
      delete;
  // ---------------------------------AddSensorData不是继承多态:因为参数列表不同，参数列表函数名相同、功能不同才是多态-----------------------
  // 处理雷达点云数据
  void AddSensorData(
