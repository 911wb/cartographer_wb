 ...
 * @param[in] wrapped_trajectory_builder 完整的slam算法 GlobalTrajectoryBuilder
 */
CollatedTrajectoryBuilder::CollatedTrajectoryBuilder(
    ...
    std::unique_ptr<TrajectoryBuilderInterface> wrapped_trajectory_builder)// 完整slam的智能指针
    // 初始化参数列表
      ...
      last_logging_time_(std::chrono::steady_clock::now()) 
{
  ...
  sensor_collator_->AddTrajectory(
      ...
      {
        // 使得数据经过sensor_collator_处理处理后，传回CollatedTrajectoryBuilder
        // 处理 按照时间顺序分发出来的传感器数据
...
// 指向父类(Data)的智能指针data，接收子类(Dispatchable)的智能指针，所以data可处理多种不同数据类型的数据
void CollatedTrajectoryBuilder::AddData(std::unique_ptr<sensor::Data> data) 
{
  // 数据收集器有很多队列，每一种队列中存放一种传感器数据，每个队列都有一种回调函数
    ...
    const std::string& sensor_id, std::unique_ptr<sensor::Data> data) 
{
  ...
  // notice: 将排序好的数据(data不同的传感器数据)送入 GlobalTrajectoryBuilder中的AddSensorData()函数中进行使用
  // 最早的data最先调用该函数
