...
// notice: 类模板不支持实参推演, 所以在类外指定模板参数的具体类型, 再进行类的实例化

// 2d的完整的slam
// (实际的返回值类型：子类指针) : (声明的返回值类型：父类指针)
...
std::unique_ptr<TrajectoryBuilderInterface> CreateGlobalTrajectoryBuilder3D(
    std::unique_ptr<LocalTrajectoryBuilder3D> local_trajectory_builder,// 前端
    const int trajectory_id, mapping::PoseGraph3D* const pose_graph,// 后端
    const TrajectoryBuilderInterface::LocalSlamResultCallback& local_slam_result_callback,// 回调
    const absl::optional<MotionFilter>& pose_graph_odometry_motion_filter) 
{
  // 返回完整slam的智能指针 ，返回子类智能指针，父类智能指针接收
  return absl::make_unique<
