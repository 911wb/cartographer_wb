...
class PoseGraph2D : public PoseGraph {
 ...
 private:
  ...
  /*
  notice:
  基于位姿图的优化运算非常消耗资源，为了不影响系统的运行实时性，每部采用了任务队列管理机制，
  可以将优化运算任务(函数)压进队列，利空空闲的片段时间分批计算。
  using WorkQueue = std::deque<WorkItem>; using WorkItem = std::function<void()>;
  指向双端队列的指针 WorkQueue = std::deque<std::function<void()>>
  需要使用work_queue_机制的任务函数：循环执行
  1.计算node约束：ComputeConstraintsForNode(
  2.优化中添加IMU数据：optimization_problem_->AddImuData(
  3.优化中添加里程计数据optimization_problem_->AddOdometryData(
  4.optimization_problem_->AddFixedFramePoseData(

  4.增加地标数据：landmark_observations.emplace_back(
  5.执行结束建图轨迹后的优化：结束建图后，要将最后的采集到的submap数据进行回环优化：DispatchOptimization()
  6.冻结轨迹：frozen_trajectories_.insert(trajectory_id)
  7.优化中添加子图数据：optimization_problem_->InsertSubmap(
  8.添加节点id：submap_data_.at(submap_id).node_ids.insert(node_id);
