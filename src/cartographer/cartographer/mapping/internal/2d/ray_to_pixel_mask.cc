...
// 计算包含线段连接部分的所有像素
std::vector<Eigen::Array2i> RayToPixelMask(const Eigen::Array2i& scaled_begin,// 原点：t_track2local
                                           const Eigen::Array2i& scaled_end,// hit点
                                           int subpixel_scale) 
{
  // For simplicity, we order 'scaled_begin' and 'scaled_end' by their x coordinate.
  // 保持起始点的x小于终止点的x
